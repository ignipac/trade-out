...
var has_move_finished: bool = true
...
func _physics_process(delta: float) -> void:
	...
	if abs(direction.x) > 0.0:
		...
		has_move_finished = false
	elif abs(direction.y) > 0.0:
		velocity.y = direction.y * speed
		has_move_finished = false
	
	if not has_move_finished:
		distance_moved += speed * delta

	if distance_moved >= grid_size:
		distance_moved = 0.0
		has_move_finished = true
		direction = Vector2.ZERO
		# can record tile travelled: useful for tile custom effects
